...
if __name__ == "__main__":
    ...
    #   设置用到的显卡
    # ------------------------------------------------------#
    # nproc_per_node：每个物理节点上⾯进程的数量。
    # 每个物理节点上的GPU的数量
    ngpus_per_node = torch.cuda.device_count()
    if distributed:
        dist.init_process_group(backend="nccl")
        # local_rank是指在⼀台机器上(⼀个node上)进程的相对序号，例如机器⼀上有0,1,2,3,4,5,6,7，机器⼆上也有0,1,2,3,4,5,6,7。local_rank在node之间相互独⽴。
        local_rank = int(os.environ["LOCAL_RANK"])
        # rank是指在整个分布式任务中进程的序号。node_rank：物理节点的序号
    ...
    if True:
        ...
        train_dataset = SiameseDataset(
            input_shape, train_lines, train_labels, True)
